...
- **CRI-kanalo**: "CRI — Ĉina Radio Internacia" (regulo 6.8: POST al la CRI-API, HLS-ludado per ExoPlayer) — videbla NUR sur Android (`videblaNurSur`); la aliaj platformoj ĝin kaŝas ĝis ekzistas transkoda servo; elŝutoj ne eblas por HLS (butono kaŝita); la elsendoj estas videoj kaj la filmotrako montriĝas en ElsendoEkrano (`VideoVido` + `VideoLudiloPonto`)
